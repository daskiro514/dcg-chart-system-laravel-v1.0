...
var Datafeeds = {};
var socket = null;
Datafeeds.UDFCompatibleDatafeed = function(socket_for_datafeed, updateFrequency) {

    socket = socket_for_datafeed;
...
Datafeeds.UDFCompatibleDatafeed.prototype.getServerTime = function(callback) {
    if (this._configuration.supports_time) {
        socket.emit('time');
        socket.on('time', function(response) {
            var time = +response;
            if (!isNaN(time)) {
                callback(time);
            }
...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
    var that = this;
    socket.emit('config');
    socket.on('config', function(response) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getMarks = function(symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
    if (this._configuration.supports_marks) {
        ...
        socket.on('marks', function(response) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getTimescaleMarks = function(symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
    if (this._configuration.supports_timescale_marks) {
        ...
        socket.on('timescale_marks', function(response) {
                onDataCallback(parseJSONorNot(response));
            })
            .on('error', function(reason) {
                onDataCallback([]);
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    ...
    if (!this._configuration.supports_group_request) {
        ...
        socket.on('symbols', function(response) {
            var data = parseJSONorNot(response);
            if (data.s && data.s !== 'ok') {
            ...
            var tmpSymbolName = data['name'];
            if (tmpSymbolName && tmpSymbolName.indexOf(':') > -1) {
                tmpSymbolName = tmpSymbolName.split(':')[1];
            }
            data['name'] = tmpSymbolName;
            data['exchange'] = "";
...
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function(symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) {
    // this._currentSymbol = symbolInfo.name.split(',')[0];
    var symbol_parts_array = symbolInfo.full_name.split(':');
    symbol_parts_array.shift();
    this._currentSymbol = symbol_parts_array.join(':');

    socket.emit('room', this._currentSymbol);
    socket.on(this._currentSymbol, function(data) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.unsubscribeBars = function(listenerGUID) {
    ...
    socket.emit('leaveroom', this._currentSymbol);
};
...
Datafeeds.SymbolsStorage.prototype._requestFullSymbolsList = function() {
    ...
    for (var i = 0; i < this._exchangesList.length; ++i) {
        ...
        socket.emit('symbol_info', {
            group: exchange
        });

        socket.on('symbol_info', (function(exchange) {
                return function(response) {
                    that._onExchangeDataReceived(exchange, parseJSONorNot(response));
                    that._onAnyExchangeResponseReceived(exchange);
                };
            })(exchange))
            .on('error', (function(exchange) {
...
Datafeeds.DataPulseUpdater.prototype.subscribeDataListener = function(symbolInfo, resolution, newDataCallback, listenerGUID) {
    ...
    socket.emit('room', symbolInfo.name);
    socket.on(symbolInfo.name, function(data) {
        onRealtimeCallback(data);
